...
export default function App({ onSwitchMode }: { onSwitchMode?: () => void }) {
  // State
  ...
  // New State: Zoom and Pan
  const [zoom, setZoom] = useState<number>(1.0);
  const [panOffset, setPanOffset] = useState<{ x: number, y: number }>({ x: 0, y: 0 });

  // New State: Dimension Line Offsets
  const [dimensionOffsets, setDimensionOffsets] = useState<{ center: number, bottom: number, top: number }>({
    center: -350,
    bottom: -600,
    top: -350
  });
  ...
  const handleAddFlushbox = () => {
    setFlushboxes(prev => {
      ...
      const label = `FB${nextNum.toString().padStart(2, '0')}`;
      const newBox: Flushbox = {
        id: crypto.randomUUID(),
        label,
        x: roomDimensions.width / 2 - 48.5, // Center horizontally
        y: roomDimensions.height - 600, // Default height (approx 600mm AFFL)
        width: 97,
        height: 62
      };
      return [...prev, newBox];
  ...
  const handleFlushboxMove = (id: string, x: number, y: number) => {
    setFlushboxes(prev => prev.map(box =>
  ...
  const handleDimensionMove = (type: 'center' | 'bottom' | 'top', offsetX: number) => {
    setDimensionOffsets(prev => ({ ...prev, [type]: offsetX }));
  };
        ...
        <aside className="w-96 flex-none bg-white border-r border-slate-200 flex flex-col overflow-y-auto z-20 shadow-lg">
          <div className="p-6 space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <button
                  onClick={handleAddFlushbox}
                  className="flex-1 flex items-center justify-center gap-2 px-3 py-2 text-xs font-medium rounded-md border border-slate-300 bg-white text-slate-600 hover:bg-slate-50 transition-colors"
                >
                  <BoxSelect size={14} /> Add Flushbox
                </button>
              </div>
